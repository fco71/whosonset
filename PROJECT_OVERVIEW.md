...
- Behavioral rules coverage added with `@firebase/rules-unit-testing`: six emulator tests cover all notification-create denials, owner read/update/delete, follow acceptance authority, mention fields, and supervisor provenance. The production Hosting workflow runs these tests before build/deploy.
...
7. Engineering backlog: expand emulator-based rules tests beyond the current notification/follow/mention coverage, Sentry, staging project, backup restore drill, ESLint, decompose ScreenplayViewer/CollaborationHub, Firestore offline persistence, indexed member search, firebase-functions package upgrade.
