...
- Behavioral rules coverage added with `@firebase/rules-unit-testing`: thirteen emulator tests cover notification authority, follow acceptance, mention fields and supervisor provenance, verified-teacher self-election, public/private profile reads, conversation message ownership, screenplay access/deletion, and the supervisor/student scene-mark deletion matrix. The production Hosting workflow runs these tests before build/deploy.
