...
Notification Phase 2 (server-side notification creation) — IN PROGRESS 2026-06-13. Goal: every in-app notification is written by a Cloud Function (Admin SDK) so the top-level `notifications` create rule can flip to `if false`, killing client-side spoofed/spam notifications. Tracked as tasks #88–95. Migration loop per event type: add/extend server trigger (deploy) → verify → remove client write (push Hosting) → verify. The win only lands at the final `create: if false` (Inc 7), so partial progress gives no security gain yet.
- Inc 1 (job applications) — DONE (audit): already 100% server-side (notifyJobApplicationCreated/StatusChange + the subcollection message trigger). The 3 client helpers in `jobApplicationService.ts` (createApplicationNotification/createStatusUpdateNotification/createMessageNotification + dead caller sendApplicationMessage) are UNREACHABLE dead code — delete in Inc 7.
- Inc 2 (chat messages) — Step A DONE + DEPLOYED + VERIFIED: `notifyNewMessage` now writes the in-app "message" notification via `createInAppNotification` (before the email throttle/preference returns, so it fires per message); sender lookup de-duped. Verified with an admin-written test message → exactly one correctly-shaped server notification. Step B (working tree, typechecks clean, ships on next push): `sendMessage` no longer calls the client `createMessageNotification` at all — so the client writes NO in-app notification AND sends NO per-message email. Per Francisco's direction, message email is now SPORADIC: the only email is the server's throttled one (≤1 per conversation per recipient / 30 min via `notifyNewMessage`), instead of one email per message. (The now-dead `createMessageNotification` method is left as a stub for the Inc 7 dead-code cleanup.) Until this push deploys, chat messages briefly produce duplicate in-app notifications (server + old client).
- Inc 0 (canonical builder) — DONE: `createInAppNotification` (functions/src/index.ts) now also carries `titleKey`/`bodyKey`/`i18nParams`/`senderName`, so it can express every notification shape incl. the i18n collaboration ones. Kept in index.ts (no shared-module refactor) to avoid a circular import with workspaceInvitations.ts.
- Inc 3 (workspace invitation created) — Step A DONE + DEPLOYED + VERIFIED: new `notifyWorkspaceInvitationCreated` trigger fires on `workspaceInvitations` create → writes the invitee notification (type `workspace_invitation`, titleKey/bodyKey `collaboration.notifications.invitedToWorkspace.*`, i18nParams `{inviter, workspace, roleKey}`, link `/collaboration`, metadata). Verified via admin-written test invitation → exactly one correctly-shaped notification. Step B (client `screenplayService.ts` now creates ONLY the invitation doc, not the notification) ships on the next push; brief duplicate window until then.
- Inc 4 (collaboration screenplay notifications) — split into sub-steps:
  - 4b (review-status) — DONE + DEPLOYED + VERIFIED: new `notifyScreenplayReviewStatus` trigger (on `screenplays` update; bails unless `reviewStatus` changed) writes review_approved / review_changes_requested(+note) / review_submitted (to workspace supervisors) / review_reopened. Key trick: `submitted` with `reviewStatusUpdatedBy === uploadedBy` = author submitting (notify supervisors); else a supervisor reopened (notify author) — no client `isReopen` flag needed. Verified via admin status changes (submitted→supervisor, changes_requested→author). Client `notifyReviewStatusChange` in ScreenplayViewer gutted to a no-op (the doc still records reviewStatus/UpdatedBy/Note, which the trigger reads).
  - 4a (annotation/tag supervisor notifications + @mentions) — PENDING (triggers on screenplayAnnotations/screenplayTags create; @mention recipient resolution TBD — likely store client-resolved mentionedUserIds on the doc to avoid server-side profile fetches + a rules change).
  - collaborator_added — PENDING, handled separately (a teamMembers-diff trigger would spam on invite-accept bulk adds, so it needs a different signal).
- Inc 5–7 — pending.
...
- Fountain persistent scenes + Movie Magic metadata (2026-06-13, CLIENT LIVE via `586041b6` / Actions run `27478840515`; RULES PENDING): the Fountain editor now has a live scene navigator that recognizes `INT.`/`EXT.` headings while typing. Debounced saves reconcile headings into stable `screenplayScenes` documents; exact anchors/headings/ordinal fallback preserve IDs and metadata across inserted lines or renamed headings, removed headings are retained as hidden orphan records, and untouched automatic scene numbers renumber while edited numbers remain fixed. PDF and Fountain scenes now share Script Day, Unit, Sequence, Est. Time, Synopsis, Note, and automatically estimated page eighths; every current screenplay participant can edit metadata after the rules release, while only screenplay editors may update Fountain source anchors. Fountain INT/EXT/location/time stay source-controlled. Both CSV reports include the five production fields, and scene search includes their values. EN/ES strings and focused reconciliation/permission tests added.
