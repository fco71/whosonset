...
- New email/password signup sends a verification email, marks the crew profile with `emailVerificationRequired: true`, and routes to `/verify-email`. The app shell also redirects marked unverified accounts away from public/protected app routes until verification is complete.
