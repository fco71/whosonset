...
- `emailSend` relay restriction (2026-06-13, CODE COMPLETE, function release pending): the client now sends its Firebase ID token; the HTTP function accepts only authenticated callers, registered-account recipients, allowlisted notification templates, bounded payloads, and a server-side 1-second/50-per-day caller quota. Non-system sender names are derived from the verified token. The production workflow now deploys Firestore rules before Hosting, removing the prior manual-only rules drift. Local Firebase CLI credentials expired before the rules dry-run; GitHub Actions is the credentialed compile/deploy path for this release.
