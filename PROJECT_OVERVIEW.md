...
1. Scene feature depth, as the class uses it: (a) Final Draft-style Categories | Scene Tags | All Tags filtering — DONE in production 2026-06-12; (b) Movie Magic fields (Script Day, Unit, Sequence, Est. Time, page-eighths) IF the class does stripboard work; (c) editable metadata for Fountain-derived scenes (needs line-anchored docs — fragile to text edits, design carefully); (d) Fountain scene navigator now receives the reader's live source, but `ScenesPanel` and `FountainPages` still parse that source separately — could share one paginated parse if performance becomes measurable.
