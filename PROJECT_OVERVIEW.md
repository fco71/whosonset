...
- Live browser QA via a throwaway signup account (2026-05-31) end-to-end on production: register (localized + 6-char password, no strength meter), auto-login into app (no verification wall), workspace create, Fountain editor (live formatted preview + autosave + page count), screenplay create, FountainViewer (Teacher-Review panel, History, Open/Mine/From-teacher filters), add annotation (toast + filter counts + history increment + hub `💬1` badge), and grading-report CSV download (correct headers + BOM + a populated data row). Found + fixed a real bug in the same session: crewProfile display names were resolved via `where('uid','in',...)` but those docs are keyed by doc-id with the `uid` field omitted at signup, so the grading-report **Student** column and the viewer **collaborator/active-users** names fell back to "Crew Member <last4>". Fixed to doc-id `getDoc` lookups (commits `1b7632a6`, `09a167dc`), deployed, and re-verified live: Student column and active-users now show the real name ("QA Claude"). Sibling spots fixed: `CollaborationHub.loadTeamMembers`, `ScreenplayViewer` collaborator hydration, grading-report uploader resolution. Throwaway account `qa.claude.0531b@example.com` (uid `BQxRlYvHMdQ0JkhynP9kCxiDghu2`) was DISABLED in Firebase Auth on 2026-05-31 (via Identity Toolkit Admin REST with the gcloud user token + `x-goog-user-project` header, since ADC needed reauth). Its orphaned Firestore docs remain (crewProfile, UserCollections, "QA Test Workspace", "QA Test Screenplay", one annotation, workspaceMembership) — harmless but the public crewProfile "QA Claude" may still list in the crew directory; delete those docs if a full scrub is wanted.
