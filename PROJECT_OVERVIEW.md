...
- Join-request / add-student approval re-elevation edge (found in the 2026-06-15 review): `grantWorkspaceMembershipInTx` in `functions/src/workspaceJoinRequests.ts` grants role `member` but, unlike the trusted `respondToWorkspaceInvitation`, does NOT `arrayRemove(uid)` from `supervisorIds`/`viewerIds`. If a uid lingers in `supervisorIds` from a prior elevation (some "remove member" paths don't clear it), approving that user's join request as a member silently re-grants supervisor (effective-supervisor reads off that array). Bounded (needs a pre-existing stale array entry) but a genuine server-side re-elevation path. Fix: mirror the `supervisorIds`/`viewerIds` arrayRemove for role `member`. Code-only Cloud Function change (no data migration); needs an `approveJoinRequest`+`addStudentToWorkspace` redeploy. Lower-priority items from the same review: no throttle on join-request creation (class-scoped notification spam is possible), and `subscribeToClassDirectory`/request subscriptions log errors but show no UI signal (feature silently shows nothing if the directory is empty/unreadable).
