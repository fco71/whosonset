...
- Firebase CLI, Firebase Console, Hosting, Firestore, Storage, Functions, and secrets work must use `iam@myfilmjobs.com` for project `my-film-jobs`. Do not deploy from any other Google account.
...
- Acceptance for MVP: Francisco has a teacher/supervisor profile discoverable by students through the app's profile lookup. This lookup is separate from deployment; the Firebase deploy identity remains `iam@myfilmjobs.com`.
...
### Admin Cloud Function for account-deletion cascade (BUILT 2026-05-28 — needs deploy)

- New callable [functions/src/cleanupUserWorkspaces.ts](functions/src/cleanupUserWorkspaces.ts) (2nd-gen `onCall`, `us-central1`), exported from [functions/src/index.ts](functions/src/index.ts). Runs with admin privileges (bypasses rules), **self-only** (acts only on `request.auth.uid`). It: removes the user from every workspace's `members[]`/`memberIds[]`/`supervisorIds`/`viewerIds`/`selfElectedSupervisors`; deletes their `workspaceMemberships` docs everywhere; hard-deletes the workspaces they OWN (+ those workspaces' membership docs). Idempotent. Screenplays under a deleted workspace are intentionally left (co-authors keep access via `teamMembers`) — deeper screenplay cascade is a possible later add.
- [AuthContext.tsx](src/contexts/AuthContext.tsx) deletion step 29 now **calls the function first** (via `getFunctions(app,'us-central1')` + `httpsCallable`), and falls back to the client-side owned-workspace soft-delete if the function is unreachable/undeployed. Whole step stays non-fatal.
- `functions` build (`tsc`) clean; app typecheck + 39 tests green.
- **ACTION for Francisco**: `firebase deploy --only functions:cleanupUserWorkspaces` (or all functions). Until deployed, account deletion uses the soft-delete fallback (works, just less thorough). **Test on a throwaway account before relying on it** — it hard-deletes owned workspaces.
...
- **Editor live preview**: the editor is now split — textarea on the left, **live formatted FountainPages preview on the right** (toggle to hide via the "Hide preview" header button). This is the "see page numbers + breaks while writing" Francisco wanted; true in-textarea breaks aren't possible, but the preview updates as you type. Editor widened to `min(1200px)`. **Resizable**: a `col-resize` divider between the two panes drags the split (editorPanePct, clamped 20–80%); hiding the preview gives the textarea 100%. (Added 2026-05-27 on Francisco's request.)
