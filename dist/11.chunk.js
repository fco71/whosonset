...
const createJobPosting = async (jobData, userId) => {
    try {
        ...
        const docRef = await (0,firebase/* addDoc */.gS)((0,firebase/* collection */.rJ)(firebase.db, 'jobs'), jobPosting);
        return docRef.id;
    }
    catch (error) {
...
const getJobPostingById = async (jobId) => {
    try {
        const docRef = (0,firebase/* doc */.H9)(firebase.db, 'jobs', jobId);
        const docSnap = await (0,firebase/* getDoc */.x7)(docRef);
        if (!docSnap.exists()) {
            // Fallback to check jobPostings collection for backward compatibility
            const legacyDocRef = (0,firebase/* doc */.H9)(firebase.db, 'jobPostings', jobId);
            const legacyDocSnap = await (0,firebase/* getDoc */.x7)(legacyDocRef);
            ...
            if (docData) {
                const newJobRef = (0,firebase/* doc */.H9)(firebase.db, 'jobs', jobId);
                await (0,firebase/* setDoc */.BN)(newJobRef, {
                    ...docData,
                    updatedAt: (0,firebase/* serverTimestamp */.O5)()
                });
                // Delete the old document
                await (0,firebase/* deleteDoc */.kd)(legacyDocRef);
            }
        ...
        if (docSnap.exists()) {
            ...
            return {
                ...data,
                id: docSnap.id,
                createdAt: data.createdAt instanceof firebase/* Timestamp */.Dc ? data.createdAt.toDate() : new Date(),
                updatedAt: data.updatedAt instanceof firebase/* Timestamp */.Dc ? data.updatedAt.toDate() : new Date()
            };
        }
        else {
            return null;
