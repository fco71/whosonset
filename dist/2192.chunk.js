...
/* harmony import */ var _utilities_emailNotificationService__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(4221);
...
const JobApplicationForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            try {
                if (job.posterEmail && currentUser.displayName) {
                    await _utilities_emailNotificationService__WEBPACK_IMPORTED_MODULE_9__/* ["default"] */ .A.sendJobApplicationNotification(job.posterEmail, job.title, currentUser.displayName, `${window.location.origin}/applications/${applicationId}`);
                }
            }
            catch (emailError) {
                console.error('Error sending job application email notification:', emailError);
                // Don't fail the application submission if email notification fails
            }
            setSuccess(true);
