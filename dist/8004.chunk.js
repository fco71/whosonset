...
const JobDetailPage = () => {
    ...
    const { jobId } = (0,dist/* useParams */.g)();
    ...
    const handleSaveJob = async () => {
        if (!auth.currentUser) {
            react_hot_toast_dist/* toast */.oR.error('Please log in to save jobs');
            return;
        }
        try {
            if (isSaved) {
                ...
                if (savedJobId) {
                    ...
                    setIsSaved(false);
                    react_hot_toast_dist/* toast */.oR.success('Job removed from saved');
                }
            }
            else {
                // Add to saved
                ...
                setIsSaved(true);
                react_hot_toast_dist/* toast */.oR.success('Job saved successfully');
            }
        }
        catch (error) {
            console.error('Error saving job:', error);
            ...
            if (error.code === 'unavailable' || error.code === 'deadline-exceeded') {
                react_hot_toast_dist/* toast */.oR.error('Connection issue. Please try again.');
            }
            else {
                react_hot_toast_dist/* toast */.oR.error('Failed to save job. Please try again.');
            }
        }
    };
    const handleShareJob = async () => {
        ...
        if (navigator.share) {
            ...
            try {
                ...
                react_hot_toast_dist/* toast */.oR.success('Job link copied to clipboard');
            }
            catch (error) {
                console.error('Error copying to clipboard:', error);
                react_hot_toast_dist/* toast */.oR.error('Failed to copy link');
            }
        }
    };
