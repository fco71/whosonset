...
const AddProject = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            try {
                if (user.email) {
                    await emailNotificationService/* default */.A.sendProjectUpdateNotification(user.email, projectName, 'created', `${window.location.origin}/projects/${legacyProjectId}`);
                }
            }
            catch (emailError) {
                console.error('Error sending project creation email notification:', emailError);
                // Don't fail the project creation if email notification fails
            }
            setIsCropping(false);
            navigate('/projects');
        }
