...
2. (certain) **Client-creatable notifications.** `users/{userId}/notifications` allows `create: if request.auth != null` (firestore.rules:371) — any user can inject notifications into any other user's inbox. Top-level `/notifications` has Phase 1 link/size hardening but is still client-creatable. Execute the Phase 2 plan already written in PROJECT_OVERVIEW.md (server-side creation, then `create: if false`), and apply the same end-state to the subcollection.
...
10. (certain) **8 test files vs 264 source files**, and `securityRules.test.ts` asserts against the rules *text* with regexes — brittle and can pass while behavior regresses. Replace with `@firebase/rules-unit-testing` against the emulator: test that a student cannot set `isTeacher`, cannot self-elect supervisor, cannot read others' notifications, cannot delete others' screenplays. Rules tests are the highest-value tests this app can have — they encode the trust model.
