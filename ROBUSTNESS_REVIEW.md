...
13. **RESOLVED 2026-06-14 — Known client-side scans.** Collaboration member search no longer downloads all `crewProfiles`: authenticated callables provide indexed prefix search and bounded ID hydration. `getProjectsForCrewMember` now uses `where('crewMemberIds', 'array-contains', uid)` and filters stale/inactive nested membership defensively. Crew mutation paths synchronize the ID field, and an idempotent production migration backfills legacy `Projects` documents before the client deploy.
